...
const CURATED_SKILLS_BY_SOURCE: Record<SourceKey, (config: ProjectConfig) => string[]> = {
  "vercel-labs/agent-skills": (_config) => {
    const skills = [
...
export async function setupSkills(
  ...
  // Load full config from bpa.jsonc to get all addons (existing + new)
  const bpaConfig = await readBpaConfig(projectDir);
  const fullConfig: ProjectConfig = bpaConfig
    ? { ...config, addons: bpaConfig.addons ?? config.addons }
    : config;
